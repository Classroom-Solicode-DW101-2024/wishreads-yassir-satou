document.addEventListener("DOMContentLoaded", () => {
  ...
  bookCards.forEach((card) => {
    card.addEventListener("click", (e) => {
      ...
      if (e.target.id === "towishlist" || e.target.closest("#towishlist"))
        return;
  ...
  function findMatchingBooks(searchTerm) {
    ...
    const bookTitles = Array.from(
      document.querySelectorAll(".book h5, .world_book h5")
  ...
  function preventDuplicateWishlistEntries() {
    ...
    bookTitles.forEach((title) => {
      if (wishlistTitles.has(title)) {
        const bookCard = Array.from(bookCards).find(
          (card) => card.querySelector("h5").textContent === title
        );
